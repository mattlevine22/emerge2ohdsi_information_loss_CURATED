...
def output_table_summary(codes_table_name, output_filename):
	...
	query_str = """select c.concept_id, c.concept_name, c.concept_code, c.vocabulary_id from public.concept as c join {codes_table_name} using (concept_id);""".format(codes_table_name=codes_table_name)
	table_output = run_query(query_str)
	df = pd.DataFrame(table_output)
	if len(df)==0:
		return
